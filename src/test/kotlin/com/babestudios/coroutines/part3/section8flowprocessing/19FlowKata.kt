...
fun <T, R> Flow<T>.mapIndexed(transformation: suspend (index: Int, T) -> R): Flow<R> {
	var index = 0
	return map { transformation(index++, it) }
}
...
fun <T> Flow<T>.withHistory(): Flow<List<T>> = scan(emptyList()) { acc, elem ->
	val k = acc.toMutableList()
	k.add(elem)
	k
}
...
fun makeLightSwitchToggle(switch1: Flow<Unit>, switch2: Flow<Unit>): Flow<Boolean> {
	var s1 = false
	return merge(switch1, switch2).map {
		s1 = !s1
		s1
	}
}
...
fun polonaisePairing(track1: Flow<Person>, track2: Flow<Person>): Flow<Pair<Person, Person>> =
	track1.zip(track2) { t1, t2 -> Pair(t1, t2) }
...
class FlowTests {
	...
	fun producingUnitsTests() = runTest {
		...
		for (i in 1..100 step 7) {
			assertEquals(List(i) { }, producingUnits(i).toList())
		}
	}
